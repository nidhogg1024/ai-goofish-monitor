...
def serialize_task(task: Task, scheduler_service, execution_queue_service=None) -> dict[str, Any]:
    ...
    if task.id is not None and execution_queue_service is not None:
        is_queued = task.id in execution_queue_service.pending_task_ids
        is_active = task.id in execution_queue_service.active_task_ids
...
def serialize_tasks(tasks: list[Task], scheduler_service, execution_queue_service=None) -> list[dict[str, Any]]:
