...
async def parse_task_intent(user_description: str) -> Dict[str, Any]:
    ...
    try:
        ...
        prompt = TASK_INTENT_PROMPT.format(user_description=description)
...
async def enrich_generate_request(req: TaskGenerateRequest) -> TaskGenerateRequest:
    ...
    task_name = _normalize_optional_text(req.task_name)
    keyword = _normalize_optional_text(req.keyword)

    if mode != "ai":
        if not task_name and keyword:
            req.task_name = keyword
        return req
    ...
    if req.min_price is None and parsed.get("min_price") is not None:
        req.min_price = parsed["min_price"]
    if req.max_price is None and parsed.get("max_price") is not None:
        req.max_price = parsed["max_price"]
    if not req.region and parsed.get("region"):
        req.region = parsed["region"]
    if not req.new_publish_option and parsed.get("new_publish_option"):
        req.new_publish_option = parsed["new_publish_option"]
    if req.personal_only is True and parsed.get("personal_only") is False:
        req.personal_only = False
    if req.free_shipping is True and parsed.get("free_shipping") is False:
        req.free_shipping = False
    if req.analyze_images is True and parsed.get("analyze_images") is False:
        req.analyze_images = False
    return req
