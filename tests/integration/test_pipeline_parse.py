...
def test_parse_search_results(load_json_fixture):
    ...
    items = _parse_search_results_json(raw, source="search")
    assert len(items) == 1
    item = items[0]
...
def test_parse_user_head_and_items(load_json_fixture):
    ...
    head = parse_user_head_data(head_json)
    assert head["卖家昵称"] == "seller_01"
    assert head["卖家收到的评价总数"] == 88

    items = _parse_user_items_data(items_json)
...
def test_parse_ratings_and_reputation(load_json_fixture):
    ...
    ratings = parse_ratings_data(ratings_json)
    assert ratings[0]["评价类型"] == "好评"

    reputation = calculate_reputation_from_ratings(ratings_json)
