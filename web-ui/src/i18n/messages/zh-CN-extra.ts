const zhCNExtra = {
  tasks: {
    title: '任务管理',
    viewDescription: '按分类与任务组管理监控池，避免不同购买目标平铺混杂。',
    defaults: {
      uncategorized: '未分类',
      defaultGroup: '默认任务组',
    },
    scope: {
      label: '当前范围',
      viewAll: '查看全部任务',
    },
    stats: {
      categoryCount: '分类数',
      groupCount: '任务组',
      taskCount: '任务总数',
      runningCount: '运行中',
    },
    category: {
      summary: '{total} 个任务，{groups} 个任务组，当前运行 {running} 个',
      groupBadge: '{count} 组',
    },
    group: {
      taskCount: '{count} 个候选任务',
      enabledCount: '已启用 {count} 个',
      runningCount: '运行中 {count} 个',
      budgetLabel: '预算',
      budgetNotSet: '未设置',
      viewResults: '查看情报',
      viewLogs: '查看日志',
    },
    toasts: {
      created: '任务创建成功',
      createFailed: '创建任务失败',
      deleted: '任务已删除',
      deleteFailed: '删除任务失败',
      updateFailed: '更新任务失败',
      descriptionRequired: '详细需求不能为空',
      regenerateFailed: '重新生成失败',
      startFailed: '启动任务失败',
      stopFailed: '停止任务失败',
      toggleFailed: '更新状态失败',
      loadAccountsFailed: '加载账号列表失败',
      notFound: '未找到要删除的任务',
      progressFailed: '任务进度获取失败',
    },
    createDialog: {
      trigger: '+ 创建新任务',
      title: '创建新监控任务（AI 或关键词）',
      description: '优先直接描述你想买什么，AI 会自动补全任务名、搜索词和基础筛选条件；下方字段都可以继续人工覆盖。',
      submit: '创建任务',
      submitting: '提交中...',
    },
    editDialog: { title: '编辑任务: {task}', save: '保存更改' },
    criteria: { title: '重新生成 AI 标准', description: '修改详细需求后将重新生成 AI 分析标准。', descriptionRequired: '请填写新的详细需求。', action: '重新生成', generating: '生成中...' },
    deleteDialog: { title: '删除任务', descriptionWithTask: '确定删除任务「{task}」吗？此操作不可恢复。', descriptionFallback: '确定删除该任务吗？此操作不可恢复。', confirm: '确认删除' },
    generation: {
      title: '任务生成进度',
      description: 'AI 正在生成分析标准并创建任务。',
      helperFailed: '生成失败后会保留这个弹窗，方便你查看错误原因。',
      helperRunning: '任务已转入后台生成，你可以停留在这里查看步骤，也可以先关闭弹窗。',
      closeWindow: '关闭窗口',
      status: { completed: '已完成', failed: '失败', running: '生成中', queued: '排队中' },
    },
    search: {
      matchingTasks: '匹配任务',
      recentTasks: '最近任务',
      placeholder: '搜索任务名、关键词或地区...',
      resultCount: '共找到 {count} 条候选任务',
      enterHint: '直接回车可打开当前高亮任务',
      loading: '正在加载任务列表...',
      emptyTitle: '没有找到匹配任务',
      emptyDescription: '试试任务名、关键词或地区，比如 MacBook、watch、上海。',
      footerHint: '结果只来自任务管理，不影响 dashboard 展示内容',
      keyboardUpDown: '↑↓ 切换',
      keyboardEnter: 'Enter 打开',
      loadFailed: '任务加载失败',
      maxPages: '最多 {count} 页',
    },
    form: {
      taskName: '任务名称',
      taskNamePlaceholder: '例如：索尼 A7M4 相机',
      taskNameAutoPlaceholder: '可选，留空时由 AI 自动生成',
      taskNameAutoHint: '不填也可以，系统会根据你的自然语言需求自动生成任务名称。',
      keyword: '搜索关键词',
      keywordPlaceholder: '例如：a7m4',
      keywordAutoPlaceholder: '可选，留空时由 AI 自动提炼闲鱼搜索词',
      keywordAutoHint: '不填也可以，系统会自动提炼更适合闲鱼搜索的关键词。',
      decisionMode: '判断模式',
      decisionModePlaceholder: '请选择判断模式',
      aiMode: 'AI判断',
      keywordMode: '关键词判断',
      description: '详细需求',
      descriptionPlaceholder: '请用自然语言详细描述你的购买需求，AI 将根据此描述生成分析标准...',
      quickCreateTitle: '现在支持自然语言直接创建',
      quickCreateHint: '只要把需求写清楚，例如“帮我蹲一台适合两只猫家庭的科沃斯 Y30，预算 700 以内”，AI 就会自动生成任务名、搜索词和基础筛选配置。',
      keywordDescriptionHint: '关键词模式下可留空；AI 模式下必填。',
      analyzeImages: '分析商品图片',
      analyzeImagesHint: '关闭后只分析商品文字描述和卖家资质，适合纯文本模型或节省 token。',
      keywordRules: '关键词规则',
      keywordRulesHint: '单组 OR 逻辑：命中任一关键词即推荐。每行一个关键词，或使用逗号分隔；纯英数字关键词按完整词匹配。',
      keywordRulesPlaceholder: '示例：a7m4\n验货宝\n全画幅',
      priceRange: '价格范围',
      minPrice: '最低价',
      maxPrice: '最高价',
      maxPages: '搜索页数',
      firstScanMaxPages: '首次扫描页数',
      firstScanMaxPagesHint: '首次执行时广撒网拉取的页数，后续增量扫描使用上方的搜索页数',
      schedule: '定时规则',
      cronPresetTab: '预设',
      cronCustomTab: '自定义',
      cronPlaceholder: '选择定时规则',
      cronCustomPlaceholder: '例如：0 8 * * * / 0 0 8 * * * / {\'@\'}daily',
      cronCustomHintLine1: '支持 5 段：分 时 日 月 周；6 段：秒 分 时 日 月 周；也支持 {\'@\'}hourly / {\'@\'}daily / {\'@\'}weekly / {\'@\'}monthly / {\'@\'}yearly。服务端时区为 Asia/Shanghai。',
      cronCustomHintLine2: '示例：每 15 分钟 */15 * * * *；每天 8 点 0 8 * * *；每天 8 点（带秒）0 0 8 * * *。',
      cron: {
        manual: '不定时（手动运行）',
        every5Minutes: '每 5 分钟',
        every15Minutes: '每 15 分钟',
        every30Minutes: '每 30 分钟',
        hourly: '每小时',
        every2Hours: '每 2 小时',
        every6Hours: '每 6 小时',
        daily8: '每天 8:00',
        daily12: '每天 12:00',
        daily18: '每天 18:00',
        daily20: '每天 20:00',
        daily81218: '每天 8:00 / 12:00 / 18:00',
        weekday9: '工作日 9:00',
        weekend10: '周末 10:00',
      },
      category: '任务分类',
      categoryPlaceholder: '例如：扫地机器人、手机数码、相机影像',
      categoryHint: '不填也可以，系统会按任务内容自动归类。',
      group: '任务组',
      groupPlaceholder: '例如：租房两猫、主力机升级、样机与库存',
      groupHint: '同一购买意图下的多个型号建议放在同一个任务组里。',
      searchQuery: '搜索词',
      searchQueryPlaceholder: '例如：追觅扫地机器人（留空则使用上方关键词搜索）',
      searchQueryHint: '用更宽泛的词搜索闲鱼，再靠关键词精准过滤型号。留空则直接用关键词搜索。',
      cronPresetHint: '默认预设会结合当前已有任务自动打散执行时间，避免新任务都堆在同一分钟触发。',
      accountStrategyLabel: '账号策略',
      fixedAccount: '指定账号',
      selectAccount: '请选择账号',
      personalOnly: '仅个人卖家',
      freeShipping: '是否包邮',
      newPublish: '新发布范围',
      region: '区域筛选（默认不填）',
      regionHint: '区域筛选会导致满足条件的商品数量很少。',
      accountStrategy: {
        auto: '自动选择',
        autoDescription: '优先使用默认登录态；无默认时使用账号池。',
        fixed: '固定账号',
        fixedDescription: '当前任务始终绑定一个指定账号。',
        rotate: '轮换账号',
        rotateDescription: '当前任务强制使用账号池轮换。',
      },
      publishOptions: { none: '不筛选（默认）', latest: '最新', oneDay: '1天内', threeDays: '3天内', sevenDays: '7天内', fourteenDays: '14天内' },
      validation: {
        incomplete: '信息不完整',
        nameAndKeywordRequired: '任务名称和关键词不能为空。',
        aiDescriptionRequired: 'AI 模式下详细需求不能为空。',
        keywordRequired: '关键词模式下必须填写搜索关键词。',
        keywordRuleIncomplete: '关键词规则不完整',
        keywordRuleRequired: '关键词模式下至少需要一个关键词。',
        accountStrategyIncomplete: '账号策略不完整',
        fixedAccountRequired: '固定账号模式下必须选择一个账号。',
      },
    },
    region: {
      provincePlaceholder: '选择省份 / 全国',
      cityPlaceholder: '选择城市 / 区域组',
      districtPlaceholder: '选择区 / 县',
      helper: '区域选项基于闲鱼页面抓取快照内置，不再依赖实时外站请求。',
      clear: '清空区域',
      current: '当前：{path}',
    },
    batchCreate: {
      trigger: '智能批量创建',
      title: '智能批量创建监控任务',
      description: '粘贴参考链接和/或描述你的需求，AI 会结合分析并生成多个监控任务。',
      urlLabel: '参考链接（可选）',
      urlPlaceholder: '粘贴网页链接，例如 B 站评测视频、购物攻略、装修清单等',
      urlHint: 'AI 会抓取网页内容作为参考，结合你的需求描述一起分析。',
      descriptionLabel: '需求描述（可选）',
      textPlaceholder: '描述你想监控的商品，例如：\n我想要购买一个 500 附近的扫地机器人，希望结合上面文章的推荐来监控',
      descriptionHint: '你的具体需求、预算、偏好等。链接和描述至少填一项。',
      startAnalyze: '开始分析',
      analyzing: '分析中...',
      progress: {
        title: 'AI 分析进度',
        description: 'AI 正在分析你的需求并生成任务配置。',
        helperRunning: '分析正在后台进行，你可以停留查看进度或先关闭弹窗。',
      },
      preview: {
        title: '任务预览',
        taskCount: '共 {count} 个任务',
        selectAll: '全选',
        deselectAll: '取消全选',
        expand: '展开编辑',
        collapse: '收起',
        taskName: '任务名称',
        keyword: '搜索关键词',
        priceRange: '价格范围',
        description: '详细需求',
        personalOnly: '仅个人卖家',
        freeShipping: '包邮',
        analyzeImages: '分析图片',
      },
      unnamedTask: '未命名任务',
      resultCreated: '已创建',
      resultUnknown: '未知',
      confirm: '创建 {count} 个任务',
      creating: '批量创建中...',
      resultSuccess: '{success}/{total} 个任务创建成功',
      resultAllSuccess: '全部 {count} 个任务创建成功！',
      resultPartialFail: '{success}/{total} 个任务创建成功，{fail} 个失败。',
      backToInput: '返回修改',
      close: '关闭',
      done: '完成',
      retry: '重新分析',
      validation: {
        contentRequired: '请至少填写参考链接或需求描述。',
        urlInvalid: '请输入有效的 URL 地址（以 http:// 或 https:// 开头）。',
        noTaskSelected: '请至少选择一个任务。',
      },
    },
    table: {
      headers: { status: '状态', details: '任务详情', crawl: '采集规则', mode: 'AI/关键配置', schedule: '触发规则', actions: '操作' },
      syncing: '数据同步中...',
      empty: '暂无监测任务',
      accountRotate: '轮换',
      accountFixed: '固定',
      accountAuto: '自动',
      systemSelected: '系统选择',
      manualTrigger: '手动触发',
      disabled: '已禁用',
      waitingSchedule: '等待调度',
      personalOnly: '个人',
      freeShipping: '包邮',
      keywordStrategies: '{count} 组策略',
      refreshCriteria: '重刷',
      queuedWaiting: '等待队列执行',
      executing: '执行中',
      queued: '已排队',
      viewResults: '查看 {task} 的情报',
      viewLogs: '查看 {task} 的日志',
      start: '启动',
      startAll: '全部启动（{count}）',
      startingAll: '启动中...',
      stop: '停止',
      stopping: '停止中',
    },
  },
  rotation: {
    title: '账号与代理轮换',
    description: '显式配置多账号切换策略，并保留代理池控制。',
    mode: '轮换模式',
    modePlaceholder: '请选择轮换模式',
    perTask: '按任务固定',
    onFailure: '失败后轮换',
    retryLimit: '重试上限',
    blacklistTtl: '黑名单 TTL（秒）',
    loading: '正在加载轮换配置...',
    save: '保存轮换设置',
    account: { title: '账号轮换', description: '为任务提供自动账号池切换。', stateDir: '账号目录' },
    proxy: { title: '代理轮换', description: '代理池保持独立控制，和账号轮换可以叠加。', pool: '代理池（逗号分隔）' },
  },
  accounts: {
    title: '闲鱼账号管理',
    description: '推荐直接打开浏览器扫码登录，也支持手动导入登录状态 JSON。',
    add: '+ 添加账号',
    cookieGuide: {
      title: '获取闲鱼登录状态',
      description: '优先使用浏览器扫码登录，系统会自动保存状态文件；扩展导出 JSON 作为备用方案保留。',
      recommendedTitle: '推荐方式：浏览器扫码登录',
      recommendedDescription: '点击右上角“浏览器扫码登录”，输入账号名后系统会自动打开闲鱼页面，扫码成功后自动保存到账号列表。',
      stepBrowser1: '点击“浏览器扫码登录”，填写账号名称。',
      stepBrowser2: '浏览器打开后，若未自动出现二维码，请在页面右上角点击登录。',
      stepBrowser3: '扫码成功后回到本页，系统会自动保存登录状态并刷新账号列表。',
      fallbackTitle: '备用方式：扩展导出 JSON',
      step1Prefix: '安装',
      extension: '闲鱼登录状态提取扩展',
      step2Prefix: '打开并登录',
      website: '闲鱼官网',
      step3: '点击扩展图标，选择“提取登录状态”，再点击“复制到剪贴板”。',
      step4: '回到本页，点击“添加账号”，粘贴 JSON 内容并保存。',
      step5: '如果配置多账号，不要在当前窗口退出闲鱼账号，可以新开无痕窗口提取其他账号 Cookie。',
    },
    list: {
      title: '账号列表',
      description: '账号文件保存在 state/ 目录下，可绑定到任务。',
      name: '账号名称',
      file: '状态文件',
      actions: '操作',
      empty: '暂无账号',
      createTask: '创建任务',
      update: '更新',
      delete: '删除',
    },
    createDialog: {
      title: '添加闲鱼账号',
      description: '粘贴通过 Chrome 插件提取的 JSON 内容。',
      name: '账号名称',
      namePlaceholder: '例如：acc_1',
      jsonContent: 'JSON 内容',
      jsonPlaceholder: '请粘贴登录状态 JSON...',
    },
    browserLogin: {
      trigger: '浏览器扫码登录',
      title: '浏览器扫码登录',
      description: '直接打开浏览器完成扫码，系统会自动把登录状态保存成账号文件。',
      accountName: '账号名称',
      accountNamePlaceholder: '例如：main_account',
      setDefaultTitle: '同步为默认登录态',
      setDefaultDescription: '保存到 xianyu_state.json，便于未绑定账号的任务直接使用。',
      statusTitle: '当前进度',
      idleStatus: '还没有开始，请先输入账号名称并启动浏览器。',
      currentAccount: '当前账号：{name}',
      currentState: '当前状态：{status}',
      defaultStateHint: '完成后会同时更新默认登录态文件。',
      step1: '点击开始后，系统会打开一个真实浏览器窗口。',
      step2: '在浏览器中完成闲鱼扫码登录；如果没有自动出现二维码，请点击页面右上角登录。',
      step3: '登录成功后无需手动复制 JSON，页面会自动提示保存完成。',
      start: '打开浏览器并扫码',
      starting: '正在启动浏览器...',
      statuses: {
        launching: '正在启动浏览器',
        awaiting_scan: '等待扫码登录',
        saving: '正在保存登录状态',
        completed: '已完成',
        failed: '失败',
        cancelled: '已取消',
      },
    },
    editDialog: { title: '更新账号：{name}', description: '替换账号的登录状态 JSON。' },
    deleteDialog: { title: '删除账号', description: '确认删除账号 {name} 吗？该操作不可恢复。', deleting: '删除中...' },
    toasts: {
      loadFailed: '加载账号失败',
      loadContentFailed: '加载账号内容失败',
      incomplete: '信息不完整',
      createDescriptionRequired: '请填写账号名称并粘贴 JSON 内容。',
      browserLoginNameRequired: '请先填写账号名称。',
      browserLoginStarted: '浏览器已打开，请完成扫码登录。',
      browserLoginStartFailed: '启动浏览器扫码失败',
      browserLoginCompleted: '扫码登录完成，账号已保存。',
      browserLoginFailed: '扫码登录失败',
      browserLoginCancelled: '扫码登录已取消',
      browserLoginStatusFailed: '获取扫码登录状态失败',
      created: '账号已添加',
      createFailed: '添加账号失败',
      contentRequired: '内容不能为空',
      updateDescriptionRequired: '请粘贴 JSON 内容。',
      updated: '账号已更新',
      updateFailed: '更新账号失败',
      deleted: '账号已删除',
      deleteFailed: '删除账号失败',
      invalidJson: 'JSON 格式错误',
      invalidJsonDescription: '请粘贴有效的 JSON 内容。',
    },
  },
  notifyPanel: {
    title: '通知推送设置',
    description: '按渠道单独配置、测试和清空。敏感字段不会回显，留空表示保留现有值。',
    noActiveChannels: '尚未配置可用通知渠道',
    enabledChannels: '已启用：{channels}',
    supportedVariables: '支持变量：title / content / price / reason / desktop_link / mobile_link',
    globalBehavior: '全局行为',
    globalBehaviorDescription: '统一控制所有渠道中的商品链接展示方式。',
    preferMobileLink: '优先附带手机端链接',
    configurationNotes: '配置说明',
    configurationNotesDescription: 'Webhook 的 Query / Body 支持 JSON 模板，测试按钮会直接调用后端真实发送逻辑，能提前发现 token、URL、JSON 格式问题。',
    loading: '正在加载通知配置...',
    clear: '清空',
    test: '测试',
    testThisChannel: '测试此渠道',
    testAll: '测试全部已启用渠道',
    save: '保存通知设置',
    footerHint: '保存会只提交改动字段；清空操作会显式删除对应渠道配置。',
    secretPlaceholder: '已配置则留空保留，输入新值覆盖',
    secretKeepPlaceholder: '已配置则留空保留',
    notConfigured: '尚未配置。',
    ntfy: {
      description: '适合轻量推送，URL 非敏感，可直接回显和修改。',
    },
    bark: {
      description: 'URL 含设备 key，已改为不回显模式。',
      configuredHint: '已配置敏感值，当前页面不回显。',
    },
    gotify: {
      description: 'URL 与 Token 必须成对配置。',
    },
    wecom: {
      title: '企业微信机器人',
      description: 'Bot URL 含 key，不回显，仅支持更新或清空。',
      urlLabel: '企业微信 Bot URL',
      configuredHint: '已保存机器人地址。',
    },
    telegram: {
      description: 'Bot Token 属于敏感字段，Chat ID 与反代地址可直接查看和修改。',
      chatIdPlaceholder: '例如：123456789',
      apiBaseUrl: 'API / 反代地址',
    },
    webhook: {
      title: '通用 Webhook',
      description: '支持 JSON 模板变量；URL 和 Headers 作为敏感字段不回显。',
      urlLabel: 'Webhook URL',
      headersLabel: 'Webhook Headers (JSON)',
      headersPlaceholder: '已配置则留空保留，例如：',
      methodLabel: 'Webhook 方法',
      contentTypeLabel: 'Webhook 内容类型',
      queryLabel: 'Webhook Query 参数 (JSON)',
      queryPlaceholder: '例如：',
      bodyLabel: 'Webhook Body (JSON 模板)',
      bodyPlaceholder: '例如：',
      variablesHelp: '可用模板变量：',
    },
  },
}

export default zhCNExtra
